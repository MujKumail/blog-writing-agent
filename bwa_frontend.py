...
def bundle_zip(md_text: str, md_filename: str, images_dir: Path) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as z:
        z.writestr(md_filename, normalize_markdown_text(md_text).encode("utf-8"))
...
def _font_candidates(bold: bool = False, italic: bool = False) -> List[str]:
    windows_fonts = Path("C:/Windows/Fonts")
    if bold and italic:
        names = ["segoeuiz.ttf", "arialbi.ttf", "calibriz.ttf"]
    elif bold:
        names = ["segoeuib.ttf", "arialbd.ttf", "calibrib.ttf"]
    elif italic:
        names = ["segoeuii.ttf", "ariali.ttf", "calibrii.ttf"]
    else:
        names = ["segoeui.ttf", "arial.ttf", "calibri.ttf"]
    return [str(windows_fonts / name) for name in names]
...
def _wrap_for_pdf(draw: ImageDraw.ImageDraw, text: str, font, max_width: int) -> List[str]:
    words = text.split()
    if not words:
        return [""]
    lines: List[str] = []
...
def _markdown_blocks_for_pdf(md: str) -> List[Tuple[str, str]]:
    lines = normalize_markdown_text(md).splitlines()
...
def markdown_to_styled_pdf(md: str) -> bytes:
    ...
    fonts = {
        "h1": _load_font(36, bold=True),
        "h2": _load_font(26, bold=True),
        "h3": _load_font(21, bold=True),
        "p": _load_font(18),
        "li": _load_font(18),
        "quote": _load_font(17, italic=True),
    }
    ...
    def new_page() -> Tuple[Image.Image, ImageDraw.ImageDraw]:
        ...
        draw.rounded_rectangle((40, 40, page_w - 40, page_h - 40), radius=28, outline="#E6DED2", width=2, fill=background)
        return image, draw
    ...
    def ensure_space(required: int):
        ...
        if y + required <= page_h - margin_y:
            return
        pages.append(image)
        image, draw = new_page()
        y = margin_y

    blocks = _markdown_blocks_for_pdf(md)
    for kind, text in blocks:
        font = fonts.get(kind, fonts["p"])
        prefix = ""
        color = text_color
        line_gap = 10
        before = 8
        ...
        lines = _wrap_for_pdf(draw, prefix + text, font, usable_width - (20 if kind == "quote" else 0))
        line_height = font.size + line_gap if hasattr(font, "size") else 28
        block_height = before + after + max(1, len(lines)) * line_height
        ensure_space(block_height)
        y += before

        if kind == "quote":
            draw.line((margin_x, y, margin_x, y + len(lines) * line_height), fill="#D9B8A7", width=4)
            x = margin_x + 18
        else:
            x = margin_x
        ...
        if kind == "h1":
            ...
            y += line_height
            draw.line((margin_x, y + 2, margin_x + usable_width, y + 2), fill="#E6DED2", width=2)
            y += after
            continue
...
def read_md_file(p: Path) -> str:
    return normalize_markdown_text(p.read_text(encoding="utf-8", errors="replace"))
...
with st.sidebar:
    ...
    if not past_files:
        ...
        load_selected_blog = st.button("📂 Load Selected Blog")
...
# Layout
tab_plan, tab_evidence, tab_preview = st.tabs(
    ["🧩 Plan", "🔎 Evidence", "📝 Markdown Preview"]
)
...
if out:
    ...
    with tab_preview:
        ...
        if not final_md:
            ...
            pdf_filename = f"{safe_slug(blog_title)}.pdf"
            st.download_button(
                "⬇️ Download Markdown",
            ...
            st.download_button(
                "⬇️ Download Styled PDF",
                data=markdown_to_styled_pdf(final_md),
